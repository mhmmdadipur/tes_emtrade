...
enum FloatingDraggableAutoAlign { onlyLeft, onlyRight, both }

enum FloatingDraggableAlignPosition { left, right, top, bottom, center }
...
class FloatingDraggableWidget extends StatefulWidget {
  const FloatingDraggableWidget({
    ...
    this.alignPosition,
    this.floatingDraggableAlign = FloatingDraggableAutoAlign.both,
  ...
  final ValueChanged<FloatingDraggableAlignPosition>? alignPosition;
  final FloatingDraggableAutoAlign floatingDraggableAlign;
...
class _FloatingDraggableWidgetState extends State<FloatingDraggableWidget>
  ...
  /// used to delay changing the position of a widget.
  Timer? timer;
  ...
  void didChangeMetrics() {
    ...
    timer = Timer.periodic(const Duration(milliseconds: 200), (timer) async {
      if (widget.autoAlign) {
        switch (widget.floatingDraggableAlign) {
          case FloatingDraggableAutoAlign.onlyLeft:
            setState(() => left = 0);
            break;
          case FloatingDraggableAutoAlign.onlyRight:
            setState(() => left = width - widget.floatingWidgetWidth);
            break;
          default:
            if (left >= width / 2) {
              setState(() => left = width - widget.floatingWidgetWidth);
        ...
        if (left > width - widget.floatingWidgetWidth) {
          setState(() => left = width - widget.floatingWidgetWidth);
        }
      }
                    ...
                    AnimatedPositioned(
                      ...
                      child: GestureDetector(
                        ...
                        onPanEnd: (value) {
                          ...
                          if (widget.alignPosition != null) {
                            FloatingDraggableAlignPosition position;

                            /// Position widget on right/left
                            if (left == 0) {
                              position = FloatingDraggableAlignPosition.left;
                            } else if (left ==
                                width - widget.floatingWidgetWidth) {
                              position = FloatingDraggableAlignPosition.right;
                            } else if (top == 0) {
                              position = FloatingDraggableAlignPosition.top;
                            } else if (top ==
                                height - widget.floatingWidgetHeight) {
                              position = FloatingDraggableAlignPosition.bottom;
                            } else {
                              position = FloatingDraggableAlignPosition.center;
                            }
                            widget.alignPosition?.call(position);
                          }
                        },
